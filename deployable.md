...
> **Validation note:** the environment this was built in had no outbound access to Docker Hub or `charts.bitnami.com` (registry/chart-repo domains outside its network allowlist), so `docker build` and `helm dependency update` for the Bitnami Postgres dependency could not be exercised end-to-end there. Everything Helm-side *was* validated with `helm lint` and `helm template` (including every `values-<env>.yaml` overlay) against locally-vendored chart dependencies, which is how the two gotchas in §4 were actually caught. Run a real `docker build` and a full `helm dependency update` (with Bitnami repo access) once in a normal CI runner or dev machine before the first real deploy.
...
- [ ] Image scanning gate in CI (no HIGH/CRITICAL CVEs reach prod) — no CI pipeline exists in this repo yet; this is a follow-up, not implemented here.
